...
// 	uint32_t DEC_VAL, ABS_VAL, temp, cnt;
// 	uint32_t BIN_VAL[op_size] = {0}; 
// 	uint32_t SOC_VAL[op_size] = {0};
// 	uint32_t STC_VAL[op_size] = {0};
// 	uint32_t SAM_VAL[op_size] = {0};
...
int main(int argc, char const *argv[])
{
	...
	uint32_t res = oct_to_dec(inputs[5].value);
	printf("Octal: %o\n", inputs[5].value);
	printf("Decimal pre funct %d\n",inputs[5].value);
	printf("Decimal Post Convertion: %d\n", res);
...
uint32_t oct_to_dec(uint32_t oct)
{
	uint32_t i = 0;
	uint32_t decimal = 0;
	while (oct != 0)				/* https://www.sanfoundry.com/c-program-convert-octal-decimal/ */
    {
